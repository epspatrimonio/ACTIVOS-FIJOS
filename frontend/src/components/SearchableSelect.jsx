    ...
    <div className={`relative w-full ${isOpen ? 'z-50' : 'z-10'}`} ref={containerRef}>
      ...
      <div className="relative flex items-center w-full bg-white border border-slate-300 hover:border-slate-400 rounded-lg shadow-sm focus-within:ring-4 focus-within:ring-brand-500/10 focus-within:border-brand-600 transition-all">
        <input
          type="text"
          name={name}
          className="block w-full px-2.5 py-1.5 pr-10 bg-transparent border-none shadow-none focus:outline-none focus:ring-0 text-slate-800 text-[11.5px] font-medium placeholder-slate-400 cursor-text"
        ...
        {value && (
          <button
            ...
            className="absolute right-7 p-0.5 rounded-md hover:bg-slate-200 text-slate-400 hover:text-slate-600 transition-colors"
          >
            <X className="w-3 h-3" />
          </button>
        )}
        ...
        <button
          ...
          className="absolute right-2 text-slate-400 p-0.5 rounded-md hover:bg-slate-200/50"
        >
          ...
          {filteredOptions.length > 0 ? (
            filteredOptions.map((opt) => (
              <button
                ...
                className={`w-full px-2.5 py-1.5 text-left text-xs hover:bg-slate-50 transition-colors flex items-center justify-between ${
            ...
            <div className="px-2.5 py-1.5 text-xs text-slate-400 italic">No se encontraron resultados</div>
          )}
        </div>
      )}
    </div>
