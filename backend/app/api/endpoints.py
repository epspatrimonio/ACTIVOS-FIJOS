...
async def get_lista_sucursales(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return [
            ...
            if (s.label or '').upper().strip() not in EXCLUDE and (s.localidad or '').upper().strip() not in EXCLUDE
        ]
    except Exception as e:
...
async def list_salidas_bienes(db: AsyncSession = Depends(get_db)):
    ...
    """
    try:
        # Asegurar existencia de columnas de devolución si no se ejecutó migración
        try:
            await db.execute(text("ALTER TABLE af.fct_salida_bienes ADD COLUMN IF NOT EXISTS estado_devolucion VARCHAR(30) DEFAULT 'SALIDA';"))
            await db.execute(text("ALTER TABLE af.fct_salida_bienes ADD COLUMN IF NOT EXISTS obs_devolucion TEXT;"))
            await db.commit()
        except Exception:
            await db.rollback()
...
async def update_salida_bienes(salida_id: int, payload: dict, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        campos_editables = ["fecha_orden", "tipo_salida", "motivo", "responsable", "cargo", "ubicacion", "resp_tecnico", "observaciones", "estado_devolucion", "obs_devolucion"]
...
@router.patch("/activos/salidas/{salida_id}/devolucion", response_model=SalidaBienesResponse, tags=["Salida de Bienes"])
async def update_devolucion_salida(salida_id: int, payload: dict, db: AsyncSession = Depends(get_db)):
    """
    Actualiza únicamente el estado de retorno/devolución y su observación.
    """
    try:
        result = await db.execute(select(SalidaBienes).where(SalidaBienes.id == salida_id))
        db_salida = result.scalar_one_or_none()
        if not db_salida:
            raise HTTPException(status_code=404, detail="Orden de salida no encontrada.")
        
        if "estado_devolucion" in payload:
            db_salida.estado_devolucion = payload["estado_devolucion"]
        if "obs_devolucion" in payload:
            db_salida.obs_devolucion = payload["obs_devolucion"]
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error al actualizar la devolución: {str(e)}"
        )
