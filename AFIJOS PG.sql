...
SELECT
    ...
    COALESCE(( SELECT d.depreciacion_acumulada
          ...
          WHERE d.cod_patrimonial = a.cod_patrimonial
          ORDER BY d.periodo DESC
         LIMIT 1),
        CASE
            WHEN a.vida_util_anios IS NOT NULL AND a.vida_util_anios > 0 AND a.valor_en_libros IS NOT NULL AND a.valor_en_libros > 0 AND a.fecha_registro_contable IS NOT NULL 
            THEN LEAST(
                ROUND(
                    (a.valor_en_libros / (a.vida_util_anios * 12.0)) * 
                    GREATEST(
                        (EXTRACT(YEAR FROM age(CURRENT_DATE, a.fecha_registro_contable)) * 12 + EXTRACT(MONTH FROM age(CURRENT_DATE, a.fecha_registro_contable))), 
                        0
                    ), 
                    2
                ), 
    ...
    (vd.vencimiento_rev_tec - CURRENT_DATE) AS dias_vigencia_rev_tec,
    CASE
        WHEN vd.vencimiento_rev_tec IS NULL THEN NULL::text
        WHEN vd.vencimiento_rev_tec < CURRENT_DATE THEN 'VENCIDO'::text
        WHEN vd.vencimiento_rev_tec <= (CURRENT_DATE + 30) THEN 'POR_VENCER'::text
        ELSE 'VIGENTE'::text
    ...
    (s_latest.fecha_vencimiento - CURRENT_DATE) AS soat_dias_vigencia,
    CASE
        WHEN s_latest.fecha_vencimiento IS NULL THEN NULL::text
        WHEN s_latest.fecha_vencimiento < CURRENT_DATE THEN 'VENCIDO'::text
        WHEN s_latest.fecha_vencimiento <= (CURRENT_DATE + 30) THEN 'POR_VENCER'::text
        ELSE 'VIGENTE'::text
    END AS soat_estado,
